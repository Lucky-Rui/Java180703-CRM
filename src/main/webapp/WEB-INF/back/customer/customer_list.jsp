...
<body>
	  ...
	  <div class="layui-inline">
	    ...
	    性别：
	  <div class="layui-inline">
		  <input class="layui-input" id="searchGender" />
	  </div>
	...
	<script>
		layui.use(['table','laydate'], function() {
			...
			table.render({
			    elem: '#tableId'
			    ,url: '${ctx}/customer/pageList.action' //数据接口
			    ,page: true //开启分页
			    ,id : "layUITableId" //设定容器唯一ID，id值是对表格的数据操作方法上是必要的传递条件，它是表格容器的索引
			    ,cols: [[ //表头
			      {type: 'checkbox', fixed: 'left'}
			      ,{field: 'id', title: 'ID', sort: true, fixed: 'left'}
			      ,{field: 'name', title: '客户姓名', sort: true, fixed: 'left'}
			      ,{field: 'gender', title: '性别', sort: true, fixed: 'left'}
			      ,{field: 'officePhone', title: '办公电话', sort: true, fixed: 'left'}
			      ,{field: 'phone', title: '手机', sort: true, fixed: 'left'}
		  ...
		  table.on('tool(tableFilter)', function(obj){
		    ...
		    if(layEvent === 'detail'){
		      ...
		      layer.confirm('真的删除行么', function(index){
		    	$.ajax({
		    		url:"${ctx}/customer/deleteById.action",
		    		data:{"id":data.id},
		    		dataType:"json",
		        ...
		        console.log(data);
		    	location.href = "${ctx}/customer/getUpdatePage.action?userId="+data.id;
		    }
		  });
		  ...
		  var active = {
		    deleteAll: function(){ //获取选中数据
		       ...
		       layer.confirm('真的删除行么', function(index){
			    	$.ajax({
			    		url:"${ctx}/customer/deleteAll.action",
			    		data:{"ids":ids},
			    		dataType:"json",
			    		success:function(resp) {
			    			if(resp.code == util.SUCCESS){
			...
			//添加
			add : function() {
				location.href = "${ctx}/customer/getAddPage.action";
			}
		  };
